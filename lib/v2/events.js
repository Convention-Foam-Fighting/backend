...
const validateEventData = ({ name, start_date, end_date, timezone }) => {
  ...
  if (start_date && end_date && new Date(start_date) > new Date(end_date)) errors.push('Start date must be before end date');
  return errors;
};
  ...
  try {
    ...
    const query = `
        WITH current_date AS (
            SELECT date('now') AS toa
        ),
             events_from_today AS (
                 SELECT DISTINCT start_date
                 FROM events
                 WHERE start_date <= (SELECT * FROM current_date) and end_date >= (SELECT * FROM current_date)
             ),
             same_day_events AS (
                 SELECT DISTINCT start_date
                 FROM events
                 WHERE start_date IN (
                     ...
                     FROM events
                     WHERE start_date > (SELECT * FROM current_date)
                 )
             ),
             final_events AS (
                 SELECT start_date
...
// Update or insert convention data for an event (upsert)
router.put('/:id/convention', async (req, res) => {
  const { id } = req.params;
  ...
  try {
    ...
    await knex.transaction(async trx => {
      ...
      // Upsert: update all records within the event date range, not just null ones
      await trx('checkin')
        .where('created_at', '>=', event.startDate)
    ...
    res.json({ message: 'Convention data updated successfully for event' });
  } catch (error) {
    console.error('Error updating convention data:', error);
    res.status(500).json({ error: 'Failed to update convention data' });
  }
});
